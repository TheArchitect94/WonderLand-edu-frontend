...
export default function Page() {
  ...
  let getData = () => {
    axios
      .get(`${baseURL}/slides`)
      .then((response) => {
        setTableRows(response.data.slides);
      })
      .catch((err) => {});
  };

  const handleDelete = (id) => {
    axios.delete(`${baseURL}/slides/${id}`).then((response) => {
  ...
  return ( 
      <div>
        <Box>
          <CSHeading Heading={"Slides Section"} Tagline={"Edit Slides"} />
        </Box>
        <Box sx={{ p: 3 }}>
